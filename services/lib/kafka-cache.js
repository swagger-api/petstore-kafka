...
module.exports = class KafkaCache {
  ...
  async saveToCache(consumer, { topic, partition, message })  {
    ...
    if(!id)
      return
    const resource = this.db.dbGet(id) || {}
    const toCache = this.onCache(resource, log, id, { topic, partition, message })
    this.db.dbPut(id, toCache)
    this.db.dbPutMeta(`${topic}-partition-0-offset`, message.offset + 1) // Note: If no new logs are added when we start up again, this will result in an invalid offset.
  }
